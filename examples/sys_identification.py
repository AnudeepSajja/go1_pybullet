            ...
            if link.name in self._link_names:
                for visual in link.visuals:
                    ...
                    if isinstance(geometry, Box):
                        ...
                        if geometry.filename.startswith("package://robot_properties_go1/"):
                            rel_path = geometry.filename[len("package://robot_properties_go1/"):]
                            base_path = "/home/anudeep/devel/workspace/src/robot_properties_go1/src/robot_properties_go1"
                            mesh_path = os.path.join(base_path, rel_path)
                        elif os.path.isfile(geometry.filename):
                            mesh_path = geometry.filename  # Already an absolute valid path
                        else:
                            raise ValueError(f"Unexpected mesh path: {geometry.filename}")
                        ...
                        origin = visual.origin.xyz if visual.origin else [0, 0, 0]

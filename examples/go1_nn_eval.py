...
try:
    ...
    for o in range(simulation_time):
        ...
        des_q = xs[pln_ctr][:pin_robot.model.nq].copy()
        des_dq = xs[pln_ctr][pin_robot.model.nq:].copy()
        des_motor_q = np.array(des_q[7:])
        des_motor_dq = np.array(des_dq[6:])

        feedforward_torques = predicted_torques_numpy

        # feedback torques
        feedback_tau = kp @ (des_motor_q - motor_pos) + kd @ (des_motor_dq - motors_vel)

        # final torques
        fin_tau = feedforward_torques + feedback_tau


        predicted_actions_list.append(feedforward_torques)

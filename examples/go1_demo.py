...
# estimator_model = '/home/anudeep/devel/workspace/models/estimator_predictor_new.pth'
# action_model = '/home/anudeep/devel/workspace/models/action_predictor_new.pth'
end2end_test_model = '/home/anudeep/devel/workspace/models/end2end_test.pth'

# estimator = torch.load(estimator_model, map_location=torch.device('cpu'))   
# action = torch.load(action_model, map_location=torch.device('cpu'))
...
try:
    ...
    for o in range(simulation_time):
        ...
        estimator_input = np.concatenate([base_pos[2], base_orn, base_vel, imu_acc, imu_gyro, motor_pos, motors_vel, foot_contact])
        ...
        # action_input = np.concatenate([predicted_state_numpy, motor_pos, motors_vel])
        # action_inputtensor = torch.tensor(action_input, dtype=torch.float32)

        # # Get the predicted action from the action model
        # action.eval()
        # with torch.no_grad():
        #     predicted_action = action(action_inputtensor)

        # predicted_action_numpy = predicted_action.cpu().numpy()  

        print("Predicted Torques: ", predicted_torques_numpy)
